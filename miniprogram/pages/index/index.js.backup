...
Page({
  ...
  onReady: function () {
    ...
    const tryInit = (attemptsLeft) => {
      console.log(`DEBUG: 尝试查找canvas (剩余尝试次数: ${attemptsLeft})`);
      ...
      const tryNextSelector = (selectorIndex) => {
        if (selectorIndex >= selectors.length) {
          ...
          if (foundCanvas) {
            ...
            proceedWithCanvas(foundCanvas);
          } else {
            if (attemptsLeft > 0) {
              console.log('DEBUG: canvas未找到，200ms后重试...');
              setTimeout(() => tryInit(attemptsLeft - 1), 200);
            } else {
              console.warn('Canvas node not found after retries with all selectors; ensure canvas exists and is visible.');
      ...
      const proceedWithCanvas = (canvas) => {
        // canvas找到，继续THREE初始化逻辑
        if (typeof createScopedThreejs === 'function') {
          try {
            console.log('DEBUG: 调用createScopedThreejs...');
            const THREE = createScopedThreejs(canvas);
          ...
          // now init 3D scene (will use global.THREE)
          this.init3DScene();
        } else {
          // fallback: attempt to require packaged dist
          try {
            const pkg = require('miniprogram_npm/threejs-miniprogram/dist/index.js');
            if (pkg && typeof pkg.createScopedThreejs === 'function') {
              createScopedThreejs = pkg.createScopedThreejs;
              const THREE = createScopedThreejs(canvas);
              try { global.THREE = THREE; } catch (err) { /* ignore */ }
              this.init3DScene();
              return;
            }
          } catch (err) {
            console.warn('fallback require for threejs-miniprogram failed', err);
          }

          try {
            const THREE = require('three');
            try { global.THREE = THREE; } catch (err) { /* ignore */ }
            console.warn('Fallback: using three with weapp-adapter shim');
            this.init3DScene();
          } catch (err) {
            console.error('threejs-miniprogram not available and three fallback failed:', err);
            this.setData({
              loading: false,
              error: {
                title: '3D 初始化失败',
                message: 'threejs-miniprogram 未就绪，且浏览器 three 无法作为回退，请在 DevTools 运行 "构建 npm" 并重试'
              }
            });
          }
        }
