...
// try to load CampusLoader robustly (support different require paths / transpilation)
let CampusLoader = null;
try {
  // prefer relative path from this page
  const mod = require('../../utils/campus-loader.js');
  CampusLoader = mod && (mod.default || mod);
} catch (e1) {
  try {
    // try project-root based path
    const mod2 = require('utils/campus-loader.js');
    CampusLoader = mod2 && (mod2.default || mod2);
  } catch (e2) {
    console.warn('CampusLoader not available via common paths', e1, e2);
  }
}
