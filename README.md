...
[![Tianjin University of Commerce](https://img.shields.io/badge/University-TUC-blue.svg)](https://www.tjcu.edu.cn/)

> 基于Three.js和微信小程序技术，专为天津商业大学打造的沉浸式3D校园虚拟旅游体验

## 正式 AppID 与部署说明

项目已配置正式小程序 AppID：`wx3352b8a1a284fe46`（写入 `miniprogram/project.config.json`）。上线前请务必完成下列后台配置，或参阅仓库根目录的 `DEPLOYMENT.md` 获取完整部署与提审清单。

- 在微信公众平台（mp.weixin.qq.com）「开发设置 / 服务器域名」中添加生产 CDN 与 API 的域名为合法域名（HTTPS）。  
- 在「开发设置 / 下载文件域名」中添加模型托管的 CDN 下载域名（用于 `wx.downloadFile`）。  
- 替换 `project.config.json` 中的 AppID（已替换为正式 AppID）；开发/测试时请使用同一 AppID 做真机测试。

更多部署步骤和检查清单见 `DEPLOYMENT.md`。
...
3. 在微信开发者工具中安装依赖：
   ```bash
   # 在微信开发者工具的项目根目录中运行
   npm install three-miniprogram weapp-adapter
   ```
4. 点击"编译"运行小程序版本

#### 小程序开发说明

- **🏫 专属定制**: 采用天津商业大学品牌色系和设计规范
- **🎨 视觉统一**: 深蓝色+金色商务配色，体现大学专业形象
- **📱 技术实现**: 使用 `three-miniprogram` 在小程序中运行 Three.js
- **💾 智能缓存**: 模型按需下载到本地存储，支持版本校验和缓存
- **🔍 LOD系统**: 支持细节级别系统，根据距离自动切换模型精度
- **📋 规范文档**: 详见 `docs/tuc_ui_guide.md` 获取完整的UI设计规范
...
基于天津商业大学官方网站设计规范，采用以下品牌色彩：

```css
/* 天津商业大学品牌色定义 */
--tuc-primary: #003366;      /* 天商深蓝 - 主色调 */
--tuc-secondary: #004080;    /* 天商商务蓝 - 辅色调 */
--tuc-accent: #1a4f7a;       /* 天商亮蓝 - 强调色 */
--tuc-gold: #d4af37;         /* 天商金色 - 装饰色 */
--tuc-light-gold: #ffd700;   /* 亮金色 - 高亮色 */
--tuc-white: #ffffff;        /* 纯白 */
```

### 设计原则

- **商务专业**: 采用深蓝色系体现商业大学的专业形象
- **简洁现代**: 扁平化设计风格，注重信息层次
- **金色点缀**: 使用金色作为装饰元素，增加视觉层次
- **响应式设计**: 适配不同设备尺寸
